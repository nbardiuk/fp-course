...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper la a ra) =
    ListZipper la (f a) ra
...
hasLeft ::
  ListZipper a
  -> Bool
hasLeft (ListZipper la _ _) = not (isEmpty la)
...
hasRight ::
  ListZipper a
  -> Bool
hasRight (ListZipper _ _ ra) = not (isEmpty ra)
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper la a ra) =
    let broke = break p la
     in case snd broke of
          (h :. t) -> IsZ (ListZipper t h (reverse (fst broke) ++ a :. ra))
          Nil      -> IsNotZ
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper la a ra) =
    let broke = break p ra
     in case snd broke of
          (h :. t) -> IsZ (ListZipper (reverse (fst broke) ++ a :. la) h t)
          Nil      -> IsNotZ
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (h :. t) a ra) =
    ListZipper t h (a :. ra)
moveLeftLoop (ListZipper Nil a ra) =
    case reverse ra of
      (h :. t) -> ListZipper (t ++ (a :. Nil)) h Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper la a (h :. t)) =
    ListZipper (a :. la) h t
moveRightLoop (ListZipper la a Nil) =
    case reverse la of
      (h :. t) -> ListZipper Nil h (t ++ (a :. Nil))
      Nil      -> ListZipper Nil a Nil
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (h :. t) a ra) =
    IsZ (ListZipper t h (a :. ra))
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper la a (h :. t)) =
    IsZ (ListZipper (a :. la) h t)
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (h :. t) a ra) =
    IsZ (ListZipper (a :. t) h ra)
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper la a (h :. t)) =
    IsZ (ListZipper la h (a :. t))
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper la a ra) =
    case reverse ra of
      (h :. t) -> ListZipper (t ++ a :. la) h Nil
      Nil      -> ListZipper la a Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper la a ra) =
    case reverse la of
      (h :. t) -> ListZipper Nil h (t ++ a :. ra)
      Nil      -> ListZipper Nil a ra
...
deletePullLeft ::
  ListZipper a
  -> MaybeListZipper a
deletePullLeft (ListZipper la _ ra) =
    case la of
      (h :. t) -> IsZ (ListZipper t h ra)
      Nil      -> IsNotZ
...
deletePullRight ::
  ListZipper a
  -> MaybeListZipper a
deletePullRight (ListZipper la _ ra) =
    case ra of
      (h :. t) -> IsZ (ListZipper la h t)
      Nil      -> IsNotZ
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a (ListZipper la oa ra) =
    ListZipper (oa :. la) a ra
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper la oa ra) =
    ListZipper la a (oa :. ra)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lf f rf) (ListZipper la a ra) =
      ListZipper (zipWith id lf la) (f a) (zipWith id rf ra)
...
instance Extend ListZipper where
  (<<=) f lz =
      ListZipper (unfold moveLeftN lz) (f lz) (unfold moveRightN lz)
          where unfold moveN = unfoldr ((next <$>) . toOptional . moveN 1)
                next a = (f a, a)
...
instance Traversable ListZipper where
    traverse afb (ListZipper la a ra) =
        let flb = traverse afb la
            frb = traverse afb ra
            fb  = afb a
         in lift3 ListZipper flb fb frb
